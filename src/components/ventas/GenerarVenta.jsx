...
const GenerarVenta = () => {
  ...
  const { data: articulosDisponibles } = useFetchData(obtenerArticulos, [
    updateKey,
  ])
  ...
  const total = useMemo(() => {
    return detalleArticulos.reduce((acc, item) => {
      const articulo = articulosDisponibles.find(
        (a) => a.id_articulo === item.articuloId
      )
  ...
  const handleAddArticulo = (articuloSeleccionado = null) => {
    setDetalleArticulos((prev) => {
      const articuloId =
        articuloSeleccionado == null ? 0 : articuloSeleccionado.id_articulo
      ...
      if (existe) {
        return prev.map((a) => {
          if (a.articuloId === articuloId) {
            const articuloData = articulosDisponibles.find(
              (x) => x.id_articulo === articuloId
            )
  ...
  const handleAddVenta = async (e) => {
    ...
    for (const { articuloId, cantidad } of detalleArticulos) {
      ...
      if (
        ...
        toast.error(
          'Completá todos los campos correctamente y respetá el stock'
        )
        return
      }
    }
        ...
        <Modal modalRef={modalRef}>
          ...
          >
            ...
            <ButtonLayout
              ...
              title={
                todosSeleccionados
                  ? 'Ya se agregaron todos los artículos disponibles'
                  : ''
              }
            >
              Agregar artículo
            ...
            <div className='flex flex-col gap-4 max-h-64 overflow-y-auto'>
              {detalleArticulos.map((articulo) => {
                const articuloData = articulosDisponibles.find(
                  (a) => a.id_articulo === articulo.articuloId
                )
                  ...
                  >
                    ...
                    <input
                      ...
                      className={`border rounded-md p-2 ${
                        esInvalido ? 'border-red-500' : 'border-gray-300'
                      }`}
            ...
            <div>
              <p className='text-left font-semibold mt-2'>Total: ${total}</p>
            </div>
