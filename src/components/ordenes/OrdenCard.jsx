    ...
    >
      <div className='flex justify-between items-center'>
        <h3 className='text-xl font-bold text-gray-800 mb-2'>
          Orden #{orden.id_orden_compra}
        </h3>
        {onClick && (
          <button onClick={() => onClick(orden)} className='cursor-pointer'>
      ...
      <p className='text-gray-700'>
        <strong>Proveedor:</strong> {orden.proveedorArticulo.proveedor.apellido}{' '}
        {orden.proveedorArticulo.proveedor.nombre}
      </p>

      <p className='text-gray-700'>
        <strong>Artículo:</strong>{' '}
        {orden.proveedorArticulo.articulo.descripcion}
      </p>

      <p className='text-gray-700'>
        <strong>Fecha estimada de recepción:</strong>{' '}
        {new Date(orden.fecha_estimada_recepcion).toLocaleDateString()}
      </p>
      ...
      <p className='text-gray-700'>
        ...
        {orden.estadoOrdenCompra.nombre_eoc.toUpperCase()}
      </p>
    </div>
  )
}
